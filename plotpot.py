...
days = glob.glob(f"Data-Hartslag-{name}/*")
amt_days = len(days)
...
data_hours = glob.glob(f"Data-Hartslag-{name}/{current_day}/*.csv")
hour_index = 0
...
def update_plot():
    ...
    ax.set_title(f"{current_day} - {name}") # Adjust the title for the correct person and day
    ...
    heart_beat = list(map(float, df["Date"][2:]))
